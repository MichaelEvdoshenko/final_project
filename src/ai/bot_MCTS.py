...
class MCTS_bot(BaseBot):
    def __init__(self, game_state: Krestik_nolik()):
        ...
        if not isinstance(game_state, Krestik_nolik):
            raise TypeError("game_state должен быть Krestik_nolik")
    ...
    def evaluate_result(self, winner):
        ...
        if winner not in ["X", "O", "НИЧЬЯ"]:
            raise TypeError("winner должен быть 'X' или 'O' или 'НИЧЬЯ'")
    ...
    def down_to_tree(self):
        ...
        while (node.children
               and len(node.children) == len(sup_game.available_stats)
               and sup_game.winner is None):
            if random.random() < 0.25:
                next_node = random.choice(node.children)
            else:
                children_ubc1 = np.array([ch.UBC1 for ch in node.children])
                maximal_UBC1 = np.max(children_ubc1)
                candidates = [ch for ch in node.children
                              if ch.UBC1 == maximal_UBC1]
                next_node = random.choice(candidates)

            if not isinstance(next_node.value,
                              (list, np.ndarray)) or len(next_node.value) != 2:
                raise ValueError("Некорректный node")
            x, y = next_node.value
            if not sup_game.make_move(x, y, player):
                raise ValueError("Невозможно сделать ход")
    ...
    def simulate(self, child, sim_game: Krestik_nolik, player):
        ...
        if not isinstance(sim_game, Krestik_nolik):
            raise TypeError("sim_game должен быть Krestik_nolik")
        if player not in ["X", "O"]:
            raise ValueError("player должен быть 'X' или 'O'")
        x, y = child
        ...
        while sim_game.winner is None:
            random_ind = np.random.randint(0, len(sim_game.available_stats))
    ...
    def find_best_move(self, game_state=None):
        ...
        if not hasattr(self.game, 'available_stats') \
           or not self.game.available_stats:
