...
class GameInterface:
    def __init__(self, screen, size=3, game_mode="friend", selected_bot=None, first_turn="player"):
        self.screen = screen
        self.size = size
    ...
    def get_cell_at_pos(self, pos):
        ...
        if self.board_x <= x <= board_right and self.board_y <= y <= board_bottom:
...
class PyGameUI:
    ...
    def _create_start_screen_elements(self):
        ...
        elements['vs_friend_btn'] = Button(280, 308, 240, 50, "Играть с другом", (255, 255, 255))
        elements['vs_bot_btn'] = Button(280, 378, 240, 50, "Играть с ботом", (255, 255, 255))
        ...
        elements['mcts_rb'] = RadioButton(280, 245, "MCTS бот", bot_radio_group, True, "MCTS")
        elements['qlearning_rb'] = RadioButton(280, 280, "Q-learning бот", bot_radio_group, False, "Q_learning")
        ...
        for i in range(len(sizes)):
            ...
            if size == 3:
                is_selected = True
            rb = RadioButton(x, 490, f"{size}x{size}", size_radio_group, is_selected, size)
            size_buttons.append((rb, size))
        elements['size_buttons'] = size_buttons
        
        elements['player_first_rb'] = RadioButton(280, 560, "Я хожу первым (O)", turn_radio_group, True, "player")
        elements['bot_first_rb'] = RadioButton(280, 595, "Бот ходит первым (X)", turn_radio_group, False, "bot")
